...
export class ComputerAI {
    ...
    constructor(rinkInfo: RinkInfo, frictionCoefficients: number, rotationDisplacementAngle: number, difficulty: Difficulty) {
    ...
    public determineNextShotParameters(playerStonePositions: Array<Vector3>, computerStonePositions: Array<Vector3>): ShotParameters {
    ...
    private findClosestStone(stonePositions: Array<Vector3>) {
        ...
        if (stonePositions.length !== 0) {
            ...
            {
                if (this._rinkInfo.targetCenter.clone().sub(stonePositions[i]).length() < closestStonePosition.length()) {
